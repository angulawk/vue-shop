  ...
  <div>
    ...
    <table class="table table-hover" v-if="cart.length">
      <caption class="text-right h3">
        <b>Total:</b>
        <Price :value="Number(cartTotalQuantity)" />
      </caption>
      <thead>
        <tr>
          <th scope="col"></th>
        ...
        <tr v-for="(item, index) in cart" :key="item.product.id">
          <td class="text-center">
            <div class="btn-group" role="group" aria-label="Basic example">
              <button @click="$emit('addProduct', item.product)" class="btn btn-info">+</button>
              <button @click="$emit('removeItem', index)" class="btn btn-outline-info">-</button>
            </div>
          </td>
